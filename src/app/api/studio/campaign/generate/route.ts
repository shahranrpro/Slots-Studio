...
import { generateCampaignOutputs } from "@/lib/campaigns/service";
import { type GenerateCampaignRequest } from "@/lib/campaigns/types";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await generateCampaignOutputs(activeWorkspace.id, body);
    if (!result.success) {
    ...
    console.error("POST /api/studio/campaign/generate error:", error);
