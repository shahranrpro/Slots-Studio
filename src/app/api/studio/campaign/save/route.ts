...
import { saveCampaignOutputToAssets } from "@/lib/campaigns/service";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await saveCampaignOutputToAssets(activeWorkspace.id, outputId);
    if (!result.success) {
    ...
    console.error("POST /api/studio/campaign/save error:", error);
