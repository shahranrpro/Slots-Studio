...
import { saveTechPackToAssets } from "@/lib/production/service";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await saveTechPackToAssets(activeWorkspace.id, techPackId);
    if (!result.success) {
    ...
    console.error("POST /api/studio/production/save error:", error);
