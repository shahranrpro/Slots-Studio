...
export interface ProductionGenerationStatusProps {
  activeJob: Job | null;
}

export function ProductionGenerationStatus({ activeJob }: ProductionGenerationStatusProps) {
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="font-mono font-bold text-[var(--text-primary)]">
              {activeJob.progressLabel || "PRODUCTION PIPELINE"}
            </span>
            <Badge
              variant={
          ...
          <p className="text-[11px] text-[var(--text-secondary)] mt-0.5">
            {activeJob.inputSummary || "Manufacturing tech pack synthesis."}
          </p>
        </div>
      </div>
