...
import { type ApprovedProductContext } from "@/lib/production/types";
import { ChevronDown, ChevronUp, Sparkles, AlertCircle, ArrowUpRight } from "lucide-react";

export interface ProductionContextPanelProps {
  context: ApprovedProductContext;
  projectId: string;
}

export function ProductionContextPanel({ context, projectId }: ProductionContextPanelProps) {
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-[var(--text-secondary)] mt-1">
                Production Studio converts approved product concepts into manufacturing specifications. Please approve a concept in Product Studio to unlock Tech Pack generation.
              </p>
            </div>
          </div>
    ...
    <Card variant="subtle" className="border-[var(--border)] overflow-hidden">
      <div className="flex items-center justify-between p-3.5 bg-[var(--surface-2)]">
        <div className="flex items-center gap-2">
          ...
          <Badge variant="success">MANUFACTURING LOCKED</Badge>
        </div>
        <button
          type="button"
      ...
      {isExpanded && (
        <div className="p-4 space-y-3.5 text-xs">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-[var(--border)]">
            {/* Colorway Swatches */}
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-mono text-[var(--text-muted)] uppercase">COLORWAYS:</span>
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[10px] font-mono text-[var(--text-muted)] uppercase">SPEC TEXTILES:</span>
